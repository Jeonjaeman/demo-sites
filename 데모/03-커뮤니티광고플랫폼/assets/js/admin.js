...
(function () {
  ...
  const advColor = (id) => (D.advertisers.find((a) => a.id === id) || {}).color || "#f45757";
  ...
  function renderDashboard() {
    ...
    $("#dashRep").innerHTML = reports.slice(0, 4).map((r) => `<li><span class="dot" style="background:${r.status === "접수" ? "#f45757" : "#9aa4ba"}"></span><span class="mt">${esc(r.ref)}</span><span class="md">${r.date}</span></li>`).join("");
  }
  ...
  /* ---- v2: 소재 심사 큐 (카카오모먼트 심사 워크플로 벤치마킹) ---- */
  let creReviews = (D.creativeReviews || []).map((x) => ({ ...x }));
  function renderReviewQueue() {
    const waiting = creReviews.filter((r) => r.status !== "처리됨");
    $("#revCount").textContent = waiting.length ? waiting.length + "건 대기" : "대기 없음";
    if (!waiting.length) { $("#revQueue").innerHTML = `<div style="color:var(--muted);font-size:13.5px;padding:14px 0">심사 대기 중인 소재가 없습니다.</div>`; return; }
    $("#revQueue").innerHTML = waiting.map((r) => `<div class="rev" data-rev="${r.id}">
      <span class="thumb" style="${r.img ? `background-image:url('${r.img}')` : "background:var(--grad-2)"}">${r.img ? "" : (r.emoji || "🖼️")}</span>
      <div class="rinfo"><b>${esc(r.name)}</b> <span class="role role-admin" style="margin-left:4px">${typeLabel(r.type)}</span>
        <small>${esc(advName(r.adv))} · ${r.id} · 등록 ${r.date}</small></div>
      <div class="rbtns">
        <span class="pill ${r.status === "보류" ? "pill-review" : "pill-wait"}">${r.status === "보류" ? "보류 중" : "심사 대기"}</span>
        <button class="btn-sm" data-hold="${r.id}">보류</button>
        <button class="btn-primary" data-pass="${r.id}" style="padding:8px 16px">승인</button>
      </div>
      ${r.holdReason ? `<div class="hold-reason">⏸ 보류 사유 — ${esc(r.holdReason)} <span style="color:var(--muted)">(광고주 대시보드에 표시되어 재심사 요청 가능)</span></div>` : ""}
    </div>`).join("");
  }
  function bindReviewQueue() {
    $("#revQueue").addEventListener("click", (e) => {
      const pass = e.target.closest("[data-pass]"), hold = e.target.closest("[data-hold]");
      if (!pass && !hold) return;
      const id = pass ? pass.dataset.pass : hold.dataset.hold;
      const r = creReviews.find((x) => x.id === id);
      if (pass) {
        creReviews = creReviews.filter((x) => x.id !== id);
        toast(`'${r.name}' 소재를 승인했습니다 — 집행이 시작됩니다.`);
      } else {
        const reason = prompt("보류 사유를 입력하세요 (광고주에게 안내됩니다)", r.holdReason || "소재 문구 근거 자료 필요");
        if (reason === null) return;
        r.status = "보류"; r.holdReason = reason;
        toast(`'${r.name}' 소재를 보류 처리했습니다.`);
      }
      renderReviewQueue();
    });
  }
  ...
  renderReviewQueue(); bindReviewQueue();
