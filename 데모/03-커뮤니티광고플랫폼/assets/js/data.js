...
window.MOA = (function () {
  ...
  const ads = {
    feed: { id: "CR-1007", type: "feed", adv: "AD-01", campaign: "프레시밀 6월 런칭",
      ...
      img: "assets/img/ad-mealkit.webp" },
    ...
    bottomsheet: { id: "CR-1013", type: "bottomsheet", adv: "AD-04", campaign: "런모어 신상 드롭",
      ...
      img: "assets/img/ad-runshoe.webp" },
    slide: [
      { id: "CR-1015", type: "slide", adv: "AD-05", brand: "스테이코지 호텔", headline: "주말 호캉스 특가", body: "도심 5성급 ~38%", grad: "linear-gradient(135deg,#8b5cf6,#a78bfa)", img: "assets/img/ad-hotel.webp" },
  ...
  const advertiserSelf = {
    ...
    /* v2: 소재 심사 현황 — 카카오모먼트 심사 상태 벤치마킹 */
    reviews: [
      { id: "CR-1022", name: "프레시밀 신메뉴 팝업", type: "popup", status: "심사중", date: "2026-06-26" },
      { id: "CR-1023", name: "'국내 1위 밀키트' 피드 카드", type: "feed", status: "보류", date: "2026-06-25", holdReason: "'국내 1위' 표현 — 객관적 근거 자료 제출 필요" },
      { id: "CR-1007", name: "집밥 15분 카드", type: "feed", status: "승인", date: "2026-06-20" }
    ]
  };
  ...
  /* ---- 광고 상품(지면) 카탈로그 — 에브리타임(지면 단위 상품)·카카오모먼트 착안 ---- */
  const adProducts = [
    { key: "feed", name: "피드 카드형", pos: "커뮤니티 피드 인스트림", size: "카드 1:1.9 (이미지 16:9)", bill: "CPM 3,500원~", target: "관심사·연령", note: "일반 글과 같은 카드 문법 + AD 라벨" },
    { key: "popup", name: "전면 팝업형", pos: "앱 진입 인터스티셜", size: "290×420 오버레이", bill: "CPM 8,000원~", target: "전체·시간대", note: "세션당 1회 · 오늘 하루 보지 않기" },
    { key: "banner", name: "하단 배너형", pos: "화면 하단 고정", size: "320×100", bill: "CPM 2,000원~", target: "전체", note: "닫기 버튼 상시 제공" },
    { key: "bottomsheet", name: "바텀시트형", pos: "피드 끝 도달 시 슬라이드업", size: "풀폭 시트", bill: "CPC 350원~", target: "행동 트리거", note: "행동 유도(CTA) 특화" },
    { key: "slide", name: "하단 슬라이드형", pos: "피드 하단 가로 캐러셀", size: "카드 200×150 ×N", bill: "CPC 250원~", target: "관심사", note: "복수 소재 동시 노출" }
  ];

  /* ---- 광고 집행 프로세스 — 당근(셀프서브)·카카오(심사) 착안 ---- */
  const processSteps = [
    { t: "광고캐시 충전", d: "선충전 지갑 방식. 신규 가입 시 무상캐시 3만원 지급, 무상분 우선 소진.", tip: "무상캐시 3만원" },
    { t: "캠페인 만들기", d: "3스텝 퀵 생성 — 지면 선택, 기간·예산, 소재 입력이면 끝. 전문가용 상세 설정도 지원.", tip: "3분 완성" },
    { t: "소재 심사", d: "등록 소재는 영업일 1일 내 심사. 보류 시 사유 안내와 함께 재심사 요청 가능.", tip: "영업일 1일" },
    { t: "집행·최적화", d: "노출/클릭 Batch 트래킹, 빈도 제한(1인당 일 N회), A/B 소재 자동 분배.", tip: "빈도 제한" },
    { t: "리포트", d: "노출·클릭·CTR 실시간 대시보드와 CSV 리포트. 캠페인 종료 시 자동 만료 처리.", tip: "CSV 제공" }
  ];

  /* ---- 소재 심사 큐 (어드민) — 카카오모먼트 심사 워크플로 착안 ---- */
  const creativeReviews = [
    { id: "CR-1021", name: "런모어 썸머 러닝 카드", adv: "AD-04", type: "feed", date: "2026-06-26 14:20", status: "대기", emoji: "👟", img: "assets/img/ad-runshoe.webp" },
    { id: "CR-1020", name: "스테이코지 풀빌라 팝업", adv: "AD-05", type: "popup", date: "2026-06-26 11:05", status: "대기", emoji: "🏨", img: "assets/img/ad-hotel.webp" },
    { id: "CR-1019", name: "노바페이 송금 배너 v2", adv: "AD-02", type: "banner", date: "2026-06-25 17:40", status: "보류", emoji: "💳",
      holdReason: "‘수수료 평생 0원’ 문구 — 근거 자료 미제출. 증빙 첨부 후 재심사 요청해 주세요." }
  ];

  /* ---- 광고주 지갑 (광고캐시) — 당근 광고캐시 착안 ---- */
  const wallet = {
    balance: 1680000, freeCash: 30000, freeExpire: "2026-07-31",
    history: [
      { t: "캠페인 소진 — 프레시밀 6월 런칭", d: "2026-06-26", amt: -128400 },
      { t: "광고캐시 충전 (카드)", d: "2026-06-24", amt: 1000000 },
      { t: "캠페인 소진 — 프레시밀 6월 런칭", d: "2026-06-23", amt: -96200 },
      { t: "신규 가입 무상캐시", d: "2026-06-01", amt: 30000 }
    ]
  };
  ...
  return {
    ...
    adProducts, processSteps, creativeReviews, wallet
  };
})();
