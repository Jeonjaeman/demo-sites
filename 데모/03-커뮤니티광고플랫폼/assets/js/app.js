...
(function () {
  ...
  function logLine(kind, cr, brand) {
    const log = $("#trackLog");
    const color = kind === "CLICK" ? "#ff8f8f" : "#8fe0a8";
    const time = new Date().toLocaleTimeString("ko-KR", { hour12: false });
    const row = document.createElement("div");
    row.innerHTML = `<span style="color:#777">${time}</span> <b style="color:${color}">${kind}</b> <span style="color:#e0e0e0">${cr}</span> <span style="color:#999">${brand}</span>`;
    log.prepend(row);
  ...
  function redirect(cr, brand, cta) {
    ...
    ov.querySelector(".ov-back").style.background = "rgba(26,26,26,.88)";
    ov.classList.add("on");
  ...
  function feedAdCard(a) {
    const vis = a.img ? `background-image:url('${a.img}')` : `background:${a.grad}`;
    return `<article class="ad-card" data-cr="${a.id}" data-brand="${esc(a.brand)}">
      <span class="label"><span class="ad-badge">${a.badge}</span></span>
      <div class="ad-vis ${a.img ? "" : "no-img"}" style="${vis}" data-ad-click="${a.id}|${esc(a.brand)}|${esc(a.cta)}">
        ${a.img ? "" : '<span class="bk">🛍️</span>'}
  ...
  function slideAd(list) {
    return `<div style="margin:0 -14px;padding:2px 0">
      ...
      ${list.map((a) => `<div data-cr="${a.id}" data-brand="${esc(a.brand)}" data-ad-click="${a.id}|${esc(a.brand)}|${esc(a.headline)}" style="flex:none;width:150px;border-radius:14px;overflow:hidden;box-shadow:var(--shadow-soft);cursor:pointer">
        <div style="height:90px;${a.img ? `background:url('${a.img}') center/cover` : `background:${a.grad}`};display:grid;place-items:center;font-size:34px">${a.img ? "" : "🏷️"}</div>
  ...
  let popupFreqCapped = false; // 빈도 제한: "오늘 하루 보지 않기" (실구축 시 localStorage/유저별 서버 캡)
  function showPopupAd() {
    if (popupFreqCapped) return;
    const a = D.ads.popup;
    const pvStyle = a.img ? `background-image:url('${a.img}')` : `background:${a.grad}`;
    $("#adOverlayInner").innerHTML = `<div class="ad-popup"><span class="label"><span class="ad-badge">${a.badge}</span></span>
      <div class="pv ${a.img ? "" : "no-img"}" style="${pvStyle}">${a.img ? "" : '<span class="bk">✨</span>'}<div class="brand">${esc(a.brand)}</div><h4>${esc(a.headline)}</h4><p>${esc(a.body)}</p></div>
      <div class="pf"><button class="btn-primary" data-ad-click="${a.id}|${esc(a.brand)}|${esc(a.cta)}" style="width:100%">${esc(a.cta)}</button>
        <button class="freq" id="popupFreq">오늘 하루 보지 않기</button>
        <button class="cls" data-close-ov>닫기 — 세션당 1회 노출 · 빈도 제한 적용</button></div></div>`;
    $("#adOverlay").querySelector(".ov-back").style.background = "rgba(26,26,26,.6)";
    $("#adOverlay").classList.add("on"); impression(a.id, a.brand);
    $("#popupFreq").addEventListener("click", () => {
      popupFreqCapped = true; $("#adOverlay").classList.remove("on");
      toast("오늘 하루 이 광고가 노출되지 않습니다 (frequency cap)");
    });
  ...
  function showBottomSheetAd() {
    ...
    const svStyle = a.img ? `background-image:url('${a.img}')` : `background:${a.grad}`;
    s.innerHTML = `<div class="grab"></div><span class="label"><span class="ad-badge">${a.badge}</span></span>
      <div class="sv ${a.img ? "" : "no-img"}" style="${svStyle}">${a.img ? "" : '<span class="bk">👟</span>'}<div class="brand">${esc(a.brand)}</div><h4>${esc(a.headline)}</h4><p>${esc(a.body)}</p></div>
