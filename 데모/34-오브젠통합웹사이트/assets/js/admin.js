(() => {
  const storageKey = "obzen-admin-demo-v1";
  const seed = [
  ...
  const persist = () => {
    try {
      ...
      showToast("브라우저 저장소에 저장하지 못했습니다.");
      return false;
    }
  };
  ...
  const updateStats = () => {
    ...
    document.querySelector("[data-stat-published]").textContent = String(items.filter((item) => item.status === "게시").length).padStart(2, "0");
    document.querySelector("[data-stat-draft]").textContent = String(items.filter((item) => item.status === "임시").length).padStart(2, "0");
    document.querySelector("[data-stat-today]").textContent = String(items.filter((item) => item.updated === formatDate()).length).padStart(2, "0");
  };
  const filteredItems = () => {
  ...
  const renderRows = () => {
    rows.replaceChildren(...filteredItems().map((item) => {
      ...
      const badgeClass = item.status === "임시" ? "admin-badge is-draft" : "admin-badge";
  ...
  document.querySelector(".admin-add").addEventListener("click", (event) => openEditor(null, event.currentTarget));
  ...
  form.addEventListener("submit", (event) => {
    ...
    const id = String(formData.get("id") || globalThis.crypto?.randomUUID?.() || `demo-${Date.now()}`);
    const next = {
      id,
      type: String(formData.get("type")),
      status: formData.get("published") ? "게시" : "임시",
    ...
    const previousItems = items;
    if (index >= 0) items = items.map((item) => item.id === id ? next : item);
