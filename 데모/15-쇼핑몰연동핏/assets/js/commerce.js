(function () {
  ...
  function header() {
    ...
    return [
      ...
      "<a href='#shop'>Shop</a><a href='#wardrobe'>Wardrobe</a><a href='#account'>My fit</a><a href='#embed'>Embed</a><a href='#admin'>Admin</a>",
      "</nav>",
  ...
  function footer() {
    return [
      ...
      "<div><strong>Customer</strong><a href='#account'>마이페이지</a><a href='#wardrobe'>내 옷장</a><a href='#account/settings'>계정 설정</a><a href='#cart'>장바구니</a></div>",
      "<div><strong>Project</strong><a href='#embed'>외부몰 임베드</a><a href='#admin'>운영자 화면</a><a href='#privacy'>개인정보 처리방침</a><a href='#terms'>이용약관</a></div>",
  ...
  function shell(content, routePath) {
    const platformWidget = window.FORMEPlatform ? window.FORMEPlatform.widgetShell(routePath) : "";
    const widgetDock = platformWidget ? "<aside class='widget-dock' aria-label='핏 추천 도구'>" + platformWidget + "</aside>" : "";
    return header() + widgetDock + "<main id='main-content' tabindex='-1'>" + content + "</main>" + footer() + fitModal() + "<div class='site-toast' role='status' aria-live='polite' hidden></div>";
  }
  ...
  function accountView() {
    ...
    return [
      ...
      ,"<section class='account-platform-links'><a href='#wardrobe'><span>", icon("bag"), "</span><div><strong>내 옷장</strong><p>실제 보유 의류와 착용감을 관리합니다.</p></div>", icon("arrow"), "</a><a href='#account/settings'><span>", icon("user"), "</span><div><strong>계정 설정</strong><p>프로필, Kakao 연결, 개인정보와 탈퇴를 관리합니다.</p></div>", icon("arrow"), "</a></section>"
    ].join("");
  }
  ...
  function render() {
    ...
    else if (window.FORMEPlatform && window.FORMEPlatform.canRender(route.path)) {
      const platformRoute = window.FORMEPlatform.renderRoute(route.path);
      content = platformRoute.html;
      title = platformRoute.title;
      standalone = Boolean(platformRoute.standalone);
    }
    else content = homeView();
    document.title = title;
    document.body.className = "commerce-site";
    document.body.innerHTML = standalone ? content + "<div class='site-toast' role='status' aria-live='polite' hidden></div>" : shell(content, route.path);
    requestAnimationFrame(function () {
      document.body.classList.add("is-ready");
      const main = document.getElementById("main-content");
      if (ui.fitOpen) {
        if (document.querySelector(".commerce-header")) document.querySelector(".commerce-header").inert = true;
        main.inert = true;
        if (document.querySelector(".commerce-footer")) document.querySelector(".commerce-footer").inert = true;
  ...
  function addCart(productId, size) {
    ...
    saveState();
    if (window.FORMEPlatform) window.FORMEPlatform.record("add_to_cart", { productId: productId, size: size });
    render();
  ...
  document.addEventListener("click", function (event) {
    ...
    if (action === "open-fit") { ui.fitOpen = actionTarget.dataset.product; ui.returnFocusProduct = actionTarget.dataset.product; ui.recommendation = null; ui.analyzing = false; ui.fitError = ""; ui.fitDraft = null; if (window.FORMEPlatform) window.FORMEPlatform.record("widget_open", { productId: actionTarget.dataset.product }); render(); }
    ...
    if (action === "apply-fit") { ui.selectedSizes[actionTarget.dataset.product] = actionTarget.dataset.size; ui.restoreFitProduct = ui.returnFocusProduct; ui.fitOpen = null; ui.recommendation = null; ui.fitError = ""; if (window.FORMEPlatform) window.FORMEPlatform.record("size_applied", { productId: actionTarget.dataset.product, size: actionTarget.dataset.size }); render(); showToast("추천 사이즈를 상품에 적용했습니다."); }
  ...
  document.addEventListener("submit", function (event) {
    ...
    if (type === "fit") {
      ...
      ui.fitError = "";
      ui.analyzing = true;
      if (window.FORMEPlatform) window.FORMEPlatform.record("recommendation_start", { productId: product.id });
      render();
      window.setTimeout(function () {
        ui.analyzing = false;
        ui.recommendation = recommendSize(product, state.fitProfile.height, state.fitProfile.weight, state.fitProfile.fit);
        if (window.FORMEPlatform) window.FORMEPlatform.record("recommendation_result", { productId: product.id, size: ui.recommendation });
        render();
      }, 650);
    }
    if (type === "login") {
      ...
      saveState();
      if (window.FORMEPlatform) window.FORMEPlatform.setMember({ name: state.user.name, email: state.user.email, provider: "email" });
      render();
    ...
    if (type === "checkout") {
      ...
      state.cart = [];
      saveState();
      if (window.FORMEPlatform) window.FORMEPlatform.record("purchase", { productId: order.items[0] ? order.items[0].productId : "contour-blouson", size: order.items[0] ? order.items[0].size : "" });
  ...
  window.FORMECommerce = {
    render: render,
    applySize: function (productId, size) {
      ui.selectedSizes[productId] = size;
      if (location.hash !== "#product/" + productId) location.hash = "product/" + productId;
      else render();
    },
    setUser: function (member) {
      state.user = member ? { name: member.name, email: member.email, phone: "" } : null;
      saveState();
      render();
    }
  };
