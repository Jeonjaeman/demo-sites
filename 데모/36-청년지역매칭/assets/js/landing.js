...
(function () {
  ...
  $$('[data-split]').forEach(function (el) {
    ...
    el.innerHTML = words.map(function (w, i) { return '<span class="w"><i style="transition-delay:' + Math.min(i * 60, 500) + 'ms">' + w + '</i></span>'; }).join(' ');
  });
  var parallaxEls = $$('[data-parallax]');
  var run = function () {
    var vh = window.innerHeight || document.documentElement.clientHeight;
    $$('.g-reveal:not(.is-visible), .mask:not(.is-visible), .words:not(.is-visible)').forEach(function (el) {
      var r = el.getBoundingClientRect();
      if (r.top < vh * 0.92 && r.bottom > 0) el.classList.add('is-visible');
    });
    parallaxEls.forEach(function (el) {
      var wrap = el.closest('.lifestate-bg') || el.parentElement;
