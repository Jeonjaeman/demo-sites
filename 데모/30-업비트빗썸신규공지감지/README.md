...
업비트·빗썸 신규 공지를 **1초 이내 감지**하는 파이썬 스크립트. 이 저장소는 정적 웹 데모라, 그 감지 로직을 **브라우저에서 실행하는 실시간 콘솔 + 실제로 동작하는 납품 파이썬 코드**로 보여줍니다.
...
- ⚠️ 웹 콘솔의 공지는 **데모용 예시**입니다(감지 로직은 실제로 실행). 실거래소 공지 감지는 `python/detector.py`가 수행합니다.
